...
import os, subprocess, shlex, distutils.core, shutil
...
class Doc:
	...
	def make_html_details(self, tree):
		...
		if 'thickness' in self.root.attrib and (tree.get('type') == 'dxf' or tree.tag == 'model'):
			text_data = ET.SubElement(element_text, 'p')
	...
	def make_pictures(self):
		...
		for family in (family for family in self.root if 'img' in family.attrib): # pout toutes les familles où il faut une image
			...
			part_img_opt = img_opt + (" --camera=" + family.get('img') if family.get('img') != 'yes' else '')
					# if family.get('type') == 'stl' else False
			param = self.root.get('data')

			process.Process(part_scad_path, family, param, export_dir, self.jobs, self.openscad_path, self.verbose_lvl, part_img_opt)
			...
			if family.get('type') == 'dxf':
				...
				for part in family:
					...
					thickness = self.root.get('thickness')

					command = 'openscad ' + scad_file + ' -D \'file="' + dxf_file + '"; thickness=' \
							+ thickness + "'" + ' -o ' + op.join(img_dir, part.get('id') + '.png')

					p_list.append(utils.cmd(command, self.verbose_lvl))
