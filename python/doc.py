...
class Doc:
	...
	def make_html_details(self, tree):
		...
		if family_name != '':
			...
			link.text = family_name.capitalize()
		...
		for info in infos:
			...
			if data not in (None, '') and tree.tag != 'part' or tree.tag == 'part' and info[2]:
		...
		if 'data' in tree.attrib:
			...
			for data in (tree.get('data') + (';' + parent_data if parent_data != None else '')).split(';'):
	...
	def get_family_attribute(self, elmt, attribute):
		...
		try:
			return parent_map[elmt].get(attribute)
		except:
			return ''
