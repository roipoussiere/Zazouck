...
Zazouck is licensed under GNU GPLv3: www.gnu.org/licenses/gpl-3.0.html""")
    ...
    my_parser.add_argument('input_path', action = 'store',
        type = _correct_input,
        help = "3d model (stl file) or model directory if you want to continue a compilation.")

    my_parser.add_argument('-o', '--output-dir', action = 'store', default = None,
    ...
    my_parser.add_argument('-p', '--param-path', action = 'store', default = None,
        type = argparse.FileType('r'),
        help = "Load a parameters file from PARAM_PATH, containing the parts parameters.")
        
    my_parser.add_argument('-t', '--test', action = 'store_true', default = False,
        help = "The files are quickly compiled for testing, not able to be printed.")

    my_parser.add_argument('-i', '--infos', action = 'store_true', default = False,
        help = "Get some informations about the model.")

    my_parser.add_argument('-d', '--doc-dir', action = 'store', default = False, metavar = "DIR",
        type = argparse.FileType('w'),
        help = "Assembly instructions directory (OUTPUT_DIR/doc by default).")

    my_parser.add_argument('-na', '--no-assembled', action = 'store_false', default = True,
        dest = 'assembled', help = "Do not build the assembled model (save time).")

    my_parser.add_argument('-V', '--verbose', action = 'store', nargs = '?',
    ...
    my_parser.add_argument('-j', '--jobs', action = 'store', nargs = '?', metavar = "NB_JOBS_SLOTS",
